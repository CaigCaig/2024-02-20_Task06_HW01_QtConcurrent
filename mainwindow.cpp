...
//В MainWindow, в методе StartRace, реализовать последовательный вызов методов DoWork объектов concurRace1 и concurRace2 при помощи QtConcurrent.
...
void MainWindow::StartRace(void){
    ...
    if(ui->rb_qtConcur->isChecked()){
        auto future = QtConcurrent::run([this]() {concurRace1->DoWork(&number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value());});
